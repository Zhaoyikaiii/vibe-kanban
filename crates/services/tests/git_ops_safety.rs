...
fn merge_refuses_with_staged_changes_on_base() {
    ...
    let res = s.merge_changes(&repo_path, &worktree_path, "feature", "main", "squash", MergeStrategy::Squash);
...
fn merge_binary_conflict_does_not_move_ref() {
    ...
    let res = s.merge_changes(&repo_path, &worktree_path, "feature", "main", "merge bin", MergeStrategy::Squash);
